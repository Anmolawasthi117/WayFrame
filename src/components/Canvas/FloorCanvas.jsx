...
const FloorCanvas = () => {
  ...
  const addConnection = useProjectStore((s) => s.addLocalConnection);
    ...
    (node, e) => {
      ...
      if (mode === "connect") {
        if (!selectedNodeId) {
          ...
          saveState();
          addConnection(activeFloor.id, selectedNodeId, node.nodeId);
          setSelectedNode(null);
          setMode("select");
        }
      } else {
        setSelectedNode(
          selectedNodeId === node.nodeId ? null : node.nodeId
        );
      }
    },
    [mode, selectedNodeId, setSelectedNode, addConnection, saveState, activeFloor]
  );
  ...
  const handleMouseMove = useCallback(
    (e) => {
      ...
      updateNode(activeFloor.id, draggedNode.nodeId, {
        coordinates: {
  ...
  const getNodeStyle = useCallback(
    (node) => {
      ...
      if (isConnecting) {
        ...
        baseClasses +=
          " bg-blue-500 text-white border-blue-600 scale-110 z-20";
      } else if (isHovered) {
